...
def compute_per_dataset(dataset_name, human_path, synth_path, raw_path=None):
    ...
    if human_df is None:
        ...
        return []
    if synth_df is None:
        print(f"ATTENZIONE: file sintetico per {dataset_name} non trovato: {synth_path}. Skipping.")
        return []

    # normalize metaphor text column names
    # possible names: "Metaphor" or "metaphor"
    ...
    used_sets = build_used_sets_from_raw(raw_df, dataset_name) if raw_df is not None else {}
